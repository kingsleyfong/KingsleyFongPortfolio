        ...
        <main className="h-[100dvh] overflow-y-auto overflow-x-hidden relative w-full bg-background tracking-tight text-foreground transition-colors duration-300">
            ...
            <div className="relative z-10 w-full flex flex-col items-center max-w-5xl mx-auto px-6">
                ...
                <section id="hero" className="w-full min-h-[90vh] flex flex-col md:flex-row items-center justify-center pt-32 pb-16 gap-12">
                    ...
                    <div className="w-56 h-56 md:w-72 md:h-72 shrink-0 animate-fade-in" style={{ animationDelay: '100ms', animationFillMode: 'both' }}>
                        <div className="w-full h-full p-2 rounded-[2rem] border border-border bg-foreground/5 backdrop-blur-sm flex items-center justify-center overflow-hidden shadow-sm">
                            <img
                                src="/kingsley.png"
                ...
                <section id="about" className="w-full py-20 border-t border-border flex flex-col justify-center min-h-[80vh]">
                    ...
                    <div className="flex flex-col gap-4 w-full max-w-4xl mx-auto">
                        {experiences.map((exp) => (
                            <Link href={`/work/${exp.slug.current}`} key={exp._id} className="group relative flex items-center gap-6 md:gap-8 p-4 md:p-6 rounded-2xl border border-border/50 bg-background/30 backdrop-blur-md hover:bg-foreground/5 hover:border-foreground/20 transition-all duration-300">
                    ...
                    <div className="w-[100vw] relative left-1/2 -translate-x-1/2 mb-16 [mask-image:_linear-gradient(to_right,transparent_0,_black_128px,_black_calc(100%-128px),transparent_100%)]">
                        <ProjectTicker
                            ...
                            onSelect={(index) => {
                                ...
                                const projectSlug = projects[index].slug?.current || projectId;
                                const exp = experiences.find(e => e.projectIds.includes(projectId));
                                const targetUrl = exp ? `/work/${exp.slug.current}#${projectSlug}` : `/work/independent-projects#${projectSlug}`;
